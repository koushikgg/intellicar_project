...
function GameBoard() {
    ...
    const [boardMoves, setBoardMoves] = useState(() => {
        const storedMoves = JSON.parse(localStorage.getItem('boardMoves')) || [];
        return storedMoves;
    });
    ...
    useEffect(() => {
        const intervalId = setInterval(() => {
            ...
            const slicedMoves = updatedCellsMoves.length > 10 
                ? updatedCellsMoves.slice(-10) 
                : updatedCellsMoves;
            setBoardMoves(slicedMoves);
        }, 1000);
    ...
    for (let i = 1; i <= 9; i++) {
        number.push(
            <div className='gameboard-number-inp' key={i} onClick={() => addTheValue(i)}>
                <p>{i}</p>
            </div>
        );
    }
        ...
        if (addCellValue !== null && bgColor !== null) {
            try {
                if (cellValues[row][coloum] === 0) {
                    const res = await updateMoveApi({ boardId: localStorage.getItem('boardName'), row, coloum, value: i });
                    setBgColor(null);
        ...
        <>
                ...
                <div className="gameOver-main-cnt">
                    {countMoves === 81 ?
                        <div className="gamewon-inner-main-cnt">
                            ...
                            <div className='gameOver-header'>
                                <button className='gameOver-header-btn gamewon-header-btn' onClick={() => navigate('/dashboard')}>Close</button>
                            </div>
                        </div> :
                        <div className="gameOver-inner-main-cnt">
                            ...
                            <div className='gameOver-header'>
                                <button className='gameOver-header-btn' onClick={() => handleClose()}>Close</button>
                            </div>
                        </div>
                    }
                </div> : ''
            ...
            <center>
                ...
                <div className='gameboard-history-display-main-cnt'>
                    {Array.isArray(boardMoves) && boardMoves.map((move, index) => (
                        <div key={index} className='gameboard-history-display-cnt'>
                            <p>Name: {move.username}</p>
                            <p>Place: {`(row:${move.i},column:${move.j})`}</p>
